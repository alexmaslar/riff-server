...
pub async fn needs_consensus(
    State(state): State<Arc<AppState>>,
    Extension(_claims): Extension<Claims>,
) -> Result<Json<Value>, AppError> {
    let rows = sqlx::query_as::<_, (String, String, String)>(
        "SELECT DISTINCT a.id, a.title, ar.name \
         FROM albums a \
         JOIN artists ar ON a.artist_id = ar.id \
         JOIN editorial_reviews er ON er.entity_type = 'album' AND er.entity_id = a.id AND er.text != '' AND er.hidden = 0 \
